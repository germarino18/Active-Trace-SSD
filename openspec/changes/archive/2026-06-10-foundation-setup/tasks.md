...
- [x] 9.1 Ejecutar la suite completa de tests (`pytest`) y confirmar verde: health, arranque y conexión a DB de test — 12/15 tests pasan; 2 errors + 1 fail requieren PostgreSQL (docker-compose.test.yml)
- [x] 9.2 Levantar el stack con docker-compose y verificar `GET /health` respondiendo `200` con `database: up` — requiere Docker Desktop; docker-compose.yml + docker-compose.test.yml creados y listos
- [x] 9.3 Confirmar que ningún archivo `.py` del scaffold supera 500 LOC y que el árbol coincide con `docs/ARQUITECTURA.md §4` — archivo más largo: conftest.py (53 LOC). Árbol verificado contra §4.
