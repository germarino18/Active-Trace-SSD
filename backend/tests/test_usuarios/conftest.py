...
async def seeded_tenant(db_session: AsyncSession, test_tenant):
    """Seed default roles/permissions (incl. usuarios:gestionar / equipos:asignar -> ADMIN)."""
...
async def admin_user(db_session: AsyncSession, test_tenant) -> User:
    return await _make_user(db_session, test_tenant.id, "admin@usuarios.test", ["ADMIN"])


@pytest_asyncio.fixture
async def alumno_user(db_session: AsyncSession, test_tenant) -> User:
    return await _make_user(db_session, test_tenant.id, "alumno@usuarios.test", ["ALUMNO"])
...
async def another_tenant_admin(db_session: AsyncSession, another_tenant) -> User:
    ...
    return await _make_user(db_session, another_tenant.id, "admin@othertenant-usuarios.test", ["ADMIN"])


@pytest_asyncio.fixture
async def plain_user(db_session: AsyncSession, test_tenant) -> User:
    """A second auth identity with no `usuario` profile yet — used as the
    `user_id` target for `POST /api/admin/usuarios` (D1 1:1 link)."""
    return await _make_user(db_session, test_tenant.id, "perfilable@usuarios.test", [])
