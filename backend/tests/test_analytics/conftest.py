...
async def admin_user(db_session: AsyncSession, test_tenant) -> User:
    return await _make_user(db_session, test_tenant.id, "admin@analytics.test", ["ADMIN"])


@pytest_asyncio.fixture
async def alumno_user(db_session: AsyncSession, test_tenant) -> User:
    return await _make_user(db_session, test_tenant.id, "alumno@analytics.test", ["ALUMNO"])
...
async def another_tenant_admin(db_session: AsyncSession, another_tenant) -> User:
    ...
    return await _make_user(db_session, another_tenant.id, "admin@othertenant-analytics.test", ["ADMIN"])
