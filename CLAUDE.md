...
| **Frontend** | React / TanStack / Tailwind / E2E | `activia-trace-design`, `tailwind-design-system`, `vercel-react-best-practices`, `dashboard-crud-page`, `react-expert`, `tanstack-query`, `react-hook-form-zod`, `typescript-advanced` | ✅ instaladas |
...
| `activia-trace-design` | Design system completo **Obsidian — Precision in Darkness** de Activia-Trace: bundle de componentes React (`_ds_bundle.js`: Button, Card, StatCard, Badge, Tabs, NavItem, EmptyState…), `styles.css` con todos los tokens y fonts (Geist), UI kits con pantallas completas (login/2FA, dashboard, Mis Materias 5-tab, Mi Perfil, 403/404). Sirve tanto para producción como para mocks/prototipos. | **Scope**: Frontend — UI / Design System del proyecto. **Trigger**: Al crear o modificar cualquier componente visual del frontend, implementar features de UI, generar mocks o prototipos, o verificar consistencia con la marca Obsidian. Skill primaria de UI — reemplaza a `stitch-ui-design`. |
