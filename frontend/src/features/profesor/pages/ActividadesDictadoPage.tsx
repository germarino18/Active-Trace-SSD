              ...
              return (
                <tr key={cal.id} className="border-b border-outline-variant hover:bg-surface-container-low">
                  ...
                  <td className="px-4 py-2">
                    {isEditing ? (
                      <input type="checkbox" checked={editState.aprobado ?? false} onChange={(e) => onChangeEdit({ aprobado: e.target.checked })} className="h-4 w-4" />
                    ) : (
...
function RegistrarNotaModal({
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<RegistrarNotaForm>({
  ...
  return (
    <div className="rounded-xl border border-outline-variant bg-surface-container-lowest p-4 space-y-3">
      <h4 style={{ margin: 0, fontSize: 16, fontWeight: 600, color: 'var(--on-surface)' }}>Registrar nota</h4>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto', gap: 12, alignItems: 'end' }}>
          <div>
            <label style={{ fontSize: 12, color: 'var(--on-surface-variant)', display: 'block', marginBottom: 4 }}>Alumno *</label>
            <select
            ...
            {errors.entrada_padron_id && (
              <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--error)' }}>{errors.entrada_padron_id.message}</p>
            )}
          </div>
          <div>
            <label style={{ fontSize: 12, color: 'var(--on-surface-variant)', display: 'block', marginBottom: 4 }}>Nota</label>
            <input
              type="number"
              step="0.01"
          ...
          <div>
            <label style={{ fontSize: 12, color: 'var(--on-surface-variant)', display: 'block', marginBottom: 4 }}>Nota textual</label>
            <input
              {...register('nota_textual')}
              className="w-full rounded-lg border border-outline-variant bg-surface-container px-3 py-2 text-label-sm text-on-surface outline-none focus:border-primary"
            />
          </div>
          <div style={{ paddingBottom: 2 }}>
            <label style={{ fontSize: 12, color: 'var(--on-surface-variant)', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
              <input type="checkbox" {...register('aprobado')} className="h-4 w-4" />
              Aprobado
            </label>
          </div>
        </div>
        ...
        {mutation.isError && <p style={{ fontSize: 13, color: 'var(--error)' }}>Error al registrar la calificación</p>}
      </form>
    </div>
  );
}
...
export function ActividadesDictadoPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {registrarNotaActividadId && (
        <RegistrarNotaModal
          actividadId={registrarNotaActividadId}
          dictadoId={dictadoId!}
          padron={padron ?? []}
          onClose={() => setRegistrarNotaActividadId(null)}
        />
      )}
